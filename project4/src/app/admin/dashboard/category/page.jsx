...
const Page = () => {
  ...
  const fetchCategories = async (page, pageSize, name = "", state = "") => {
    try {
      ...
      if (response.content && Array.isArray(response.content)) {
        ...
        console.log("Pagination Metadata (Categories):", paginationMetadata);
        setMetadata(paginationMetadata);
      } else if (response.data && Array.isArray(response.data)) {
        // Fallback cho format cũ
        setCategories(response.data);
        if (response.metadata) {
          console.log("Pagination Metadata (Categories):", response.metadata);
  ...
  const handleEditSubmit = async (e) => {
    ...
    try {
      ...
      const response = await categoryService.updateCategory(selectedCategory._id || selectedCategory.id, categoryData, language);
  ...
  const handleDeleteConfirm = async () => {
    try {
      ...
      const response = await categoryService.deleteCategory(selectedCategory._id || selectedCategory.id);
