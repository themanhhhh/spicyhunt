...
const Page = () => {
  ...
  const fetchTables = async (page, pageSize) => {
    try {
      ...
      if (response.content && Array.isArray(response.content)) {
        ...
        console.log("Pagination Metadata (Tables):", paginationMetadata);
        setMetadata(paginationMetadata);

        if (page === 0) {
          console.log("Tables loaded:", response.content.length);
        }
  ...
  const handleEditSubmit = async (e) => {
    ...
    try {
      ...
      const response = await ordertableService.updateOrderTable(
        selectedTable._id || selectedTable.id,
        formData.name,
        formData.state,
  ...
  const handleDeleteConfirm = async () => {
    try {
      ...
      const response = await ordertableService.deleteOrderTable(selectedTable._id || selectedTable.id);
